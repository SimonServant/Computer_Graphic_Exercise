...
window.addEventListener('load', () => {
    ...
    const gn1 = new GroupNode(Matrix.translation(new Vector(.5, .3, 0, 0)));
    sg.add(gn1);
    const gn3 = new GroupNode(Matrix.scaling(new Vector(0.4, 0.3, 0.2, 0)));
    gn1.add(gn3);
    const sphere1 = new SphereNode(new Vector(.8, .4, .1, 1))
    gn3.add(sphere1);
    const gn2 = new GroupNode(Matrix.translation(new Vector(-.2, -0.2, 0, 0)));
    sg.add(gn2);
